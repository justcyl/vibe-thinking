...
Git 历史接近 Conventional Commits（示例：`feat: Initialize Vibe-coding project structure`），请沿用 `type: short-summary` 并在正文补充动机、风险与回退计划。PR 描述需包含：需求或 Issue 链接、变更摘要、截图/GIF（所有 UI 变更必填）、执行命令及结果、相关配置步骤（例如如何设置 `API_KEY`）。保持 PR 聚焦单一主题，并说明如何在 AI Studio 中复现和验证。
